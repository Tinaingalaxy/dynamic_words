...
def report_func(global_step, epoch, batch, num_batches,
    ...
    if batch % config['TagSampler']['Trainer']['steps_per_stats'] == -1 % config['TagSampler']['Trainer']['steps_per_stats']:
        ...
        summery_writer.add_scalar('progress/bow_loss',report_stats.loss/256,global_step)
...
def build_or_load_model(config, fields):
    ...
    model = create_tag_sampler(config,fields)
...
def train_model(model, train_data, fields, optim, lr_scheduler, start_epoch_at):
    ...
    train_loss = SamplerLossCompute(len(fields['tag'].vocab),fields['tag'].vocab.stoi[IO.PAD_WORD])
