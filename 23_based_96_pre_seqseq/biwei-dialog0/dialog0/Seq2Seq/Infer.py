...
class Infer(object):
    ...
    def inference_batch(self, batch, tag_input, tag_score):
        ...
        # print("所以传进来的是：",tag_input,tag_score)
    ...
    def inference_batch_list(self, batch, tag_input_list, tag_score_list):

        print("所以传进来的是：",tag_input_list,tag_score_list)
        src_inputs = batch.src[0]
        src_inputs = src_inputs.cuda()
        src_lengths = batch.src[1].tolist()
        # tag_inputs = batch.tag.squeeze()
        ...
        selected_tag_list = [Variable(torch.LongTensor([tag_input])).unsqueeze(-1).cuda() for tag_input in tag_input_list]
        selected_tag_idx_list = tag_input_list
        selected_tag_score_list = tag_score_list

        tag_hidden_list =[self.seq2seq_model.tag_encode(selected_tag) for selected_tag in selected_tag_list]
    ...
    def sample_tag_with_kmeans(self, batch, topk_tag=1000, n_cluster=3):
        ...
        # print("src_inputs:",src_inputs,cluster)
        return clusters
    ...
    def get_tag_init_embedding(self,tag_hidden_list,hidden_last):
